// ============================================================================
// Audio Agent Sample — Text-to-Speech output
// ============================================================================
//
// Demonstrates:
//   1. An LlmAgent configured for AUDIO response modality (TTS)
//   2. Selecting a prebuilt voice via SpeechConfig
//   3. Extracting inline audio data from the response and saving it to a .wav
// ============================================================================
...
await foreach (var evt in resultEnumerable)
{
    ...
    foreach (var part in evt.Content.Parts)
    {
        if (part.InlineData != null && part.InlineData.MimeType.StartsWith("audio/"))
        {
            ...
            const string filename = "output_audio.wav";
